...
def deduplicate_rows(rows):
    ...
    for row in rows:
        ...
        key_full = (domaine.lower(), tech.lower(), value.lower(), version.lower())
        if key_full in seen_full:
            continue
        seen_full.add(key_full)
        key_tech = (domaine.lower(), tech.lower())
        map_by_tech.setdefault(key_tech, []).append((value, version, row))

    deduped = []
    for k, variants in map_by_tech.items():
        # Correction ici : on garde bien la structure tuple
        with_val_or_ver = [(v, ver, r) for (v, ver, r) in variants if v or ver]
        if with_val_or_ver:
            local_seen = set()
            for v, ver, r in with_val_or_ver:
                key = (v.lower(), ver.lower())
                if key in local_seen:
                    continue
                local_seen.add(key)
                deduped.append(r)
        else:
            deduped.append(variants[0][2])
    return deduped
...
def main():
    ...
    all_rows = whatweb_rows + nuclei_rows
    deduped_rows = deduplicate_rows(all_rows)
    print_table(deduped_rows)
