...
def main():
    ...
    print(f"{'Domaine':<25} {'Technologie':<16} {'Valeur':<32} {'Version':<12}")
    print('-'*25 + ' ' + '-'*16 + ' ' + '-'*32 + ' ' + '-'*12)
    seen = set()
    for plugin in plugins:
        ...
        if tech.lower() in technos_json:
            ...
            domaine = plugin['domaine']
            key = (domaine.lower(), tech.lower(), val.lower())
            if key in seen:
                continue
            seen.add(key)
            print(f"{domaine:<25} {tech:<16} {val:<32} {version:<12}")
